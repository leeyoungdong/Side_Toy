...
def post_proc(pred):
    ...
    proc_pred[:, 1] = np.where((0.3 < pred[:, 0]) & (pred[:, 0] < 0.7), np.float32(0.098461), 1 - proc_pred[:, 0])
    proc_pred[:, 2] = np.where((0.3 < pred[:, 1]) & (pred[:, 1] < 0.7), np.float32(0.931948), pred[:, 1])
    proc_pred[:, 3] = np.where((0.3 < pred[:, 1]) & (pred[:, 1] < 0.7), np.float32(0.104006), 1 - proc_pred[:, 2])
